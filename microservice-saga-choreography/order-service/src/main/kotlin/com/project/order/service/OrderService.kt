...
class OrderService(
    ...
    fun placeOrder(command: PlaceOrderCommand) {
        val order = orderRepository.findById(command.orderId).orElseThrow()
        val orderItems = orderItemRepository.findAllByOrderId(order.id!!)

        order.request()
        orderRepository.save(order)

        // 커밋이 끝난 뒤에만 이벤트를 발행한다. (DB 롤백 시 유령 이벤트가 나가는 것을 방지)
    ...
    fun getStatus(orderId: Long): Order.OrderStatus {
        val order = orderRepository.findById(orderId).orElseThrow()
        return order.status
    }
}
